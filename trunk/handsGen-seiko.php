...
$sBack = ImageColorAllocate($sBan, 0xff, 0xff, 0xff);
...
// 大盤の初期化、背景を透明色で塗りつぶす
$dBack = ImageColorAllocate($dBan, 0xff, 0xff, 0xff);
...
for ($i = 0; $i < 60; $i++) {
  ...
  ImagefilledPolygon($sBan, trans4(rot4(kshift4($tanValue, rot1($kageb, -$d)), $d)), 4, $kageColor);
  ...
  ImagefilledPolygon($sBan, trans8($enValue), 8, $hariColor);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
  // 大盤上に縮小コピー
  ...
  ImageFilledRectangle($sBan, 0,0, $sBanWidth, $sBanHeight, $sBack);
  // 影の描画(逆回転した影ベクトルを用いて針をシフトし、正回転させて影を描画)
  ImagefilledPolygon($sBan, trans4(rot4(kshift4($chouValue, rot1($kageb, -$d)), $d)), 4, $kageColor);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
  // ImageFilger($sBan, IMG_FILTER_GAUSSIAN_BLUR, 1);
